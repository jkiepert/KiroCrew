...
async def _unhide_folder(state: DashboardState, folder_id: str) -> None:
    ...
    def _clear(folders: list[dict[str, Any]]) -> tuple[bool, None]:
        for f in folders:
            if f["id"] == folder_id and f.get("hidden"):
                f["hidden"] = False
                return True, None
        # Already visible (or gone): report no change so the store is not
        # rewritten. This runs on every session move, so a needless write here
        # would be a write per move.
        return False, None

    await state.mutate_folders(_clear)
