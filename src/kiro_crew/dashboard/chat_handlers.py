...
async def api_chat_slot_delete(request: web.Request) -> web.Response:
    ...
    # An app-owned slot is only the visible half of something the app drives on a
    # timer, so the app has to learn the user dismissed it — otherwise its own
    # watchdog sees a live worker with no loop, cannot tell that from a restart
    # that lost one, and re-arms the very tab that was just closed.
    #
    # BEFORE the session teardown below, not after: the dismissal is already
    # committed (the slot is out of ``_slots`` and saved to history), while
    # ``sessions.remove`` can raise on an ACP teardown error. Notifying second
    # meant such a raise propagated out of this handler with the app never told —
    # leaving a live worker whose watchdog then relaunches the closed tab, which
    # is the resurrection this hook exists to prevent.
    #
    ...
    if slot._app:
        ...
        await notify_slot_closed(slot._app, name)
