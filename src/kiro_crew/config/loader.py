...
def _config_file_lock(path: Path, *, timeout: float, blocking: bool) -> Iterator[bool]:
    ...
    decides between refusing and proceeding. If the platform offers no locking at all
    the lock is reported as acquired: a settings write must not become impossible on a
    platform without ``fcntl`` or ``msvcrt``, and the fingerprint check in
    :func:`config_transaction` still detects an interleaved write.
    """
    ...
    try:
        ...
        # Cannot even create the sidecar (read-only dir). Degrade to unlocked rather
        # than making the write impossible; the fingerprint check still applies.
        yield True
        return
...
class ConfigTransaction:
    ...
    def write(self, data: dict, *, fsync: bool = False) -> None:
        ...
        write_config_atomically(self._path, data, fsync=fsync)
...
def config_transaction(
    ...
    **Blocking is correct here and only here.** The critical section is a single atomic
    ...
    with _config_file_lock(target, timeout=timeout, blocking=True) as acquired:
        ...
        yield ConfigTransaction(target, locked=acquired)
