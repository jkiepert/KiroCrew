...
def chat_dir(run_id: str, change_id: str,
    ...
    outside. The anchor has to be a directory the attacker cannot relocate, and
    every component below it is checked, so the same trick one level higher does
    not work either.
    ...
    """
    try:
        anchor = store.runs_root(root).resolve()
