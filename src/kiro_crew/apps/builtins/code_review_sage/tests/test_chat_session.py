...
class DisableHookTests(unittest.IsolatedAsyncioTestCase):
    ...
    def setUp(self):
        # The app's own module object, NOT the `sage_lib.*` path shim this file
        # imports as CS: those are two distinct modules with separate `_REGISTRY`
        # globals, so driving the shim's registry here would assert against a
        # registry `on_disable` never touches — green while the hook does nothing.
        from kiro_crew.apps.builtins.code_review_sage.sage_lib import chat_session as appcs
        self.appcs: object = appcs
        self._real = appcs.safety_override
        appcs.safety_override = lambda: _fake_override(True)  # type: ignore[assignment]

    def tearDown(self):
        setattr(self.appcs, "safety_override", self._real)
        setattr(self.appcs, "_REGISTRY", None)

    async def test_disabling_closes_every_retained_chat(self):
        from kiro_crew.apps.builtins import code_review_sage as app
        pool = FakePool()
        reg = self.appcs.get_registry(pool)  # type: ignore[attr-defined]
        await reg.adopt("k1", FakeHandle(), "a")
        await reg.adopt("k2", FakeHandle(), "a")
        self.assertEqual(len(reg._sessions), 2)

        app.on_disable(object())
    ...
    async def test_disabling_with_no_registry_is_a_no_op(self):
        from kiro_crew.apps.builtins import code_review_sage as app
        setattr(self.appcs, "_REGISTRY", None)
        app.on_disable(object())                # must not raise
