...
function staleUpdateProvenanceIsAbsent(fs, record, artifactPath) {
  const pending = record && record.pending;
  if (!pending || !validRecovery(pending.previous) ||
      !isGeneratedSidePath(artifactPath, pending.tempPath, ["tmp"]) ||
      inspectArtifact(fs, pending.tempPath).status !== "absent") return false;
  return pending.previous.paths.every((candidate) =>
    isGeneratedSidePath(artifactPath, candidate, ["tmp", "kirocrew-displaced"]) &&
    inspectArtifact(fs, candidate).status === "absent"
  );
}
...
function recoveryForAbsent(fs, record, artifactPath, fingerprintMetadata) {
  ...
  if (record.pending && validRecovery(record.pending.previous)) {
    return staleUpdateProvenanceIsAbsent(fs, record, artifactPath)
      ? null
...
function reconcileArtifact(
  ...
  if (current.status === "absent") {
    ...
    if (record && !previous) {
      const activeAnchorIsAbsent =
        !record.pending &&
        isGeneratedSidePath(artifactPath, record.anchorPath, ["tmp"]) &&
        inspectArtifact(fs, record.anchorPath).status === "absent";
      const firstInstallPendingIsAbsent =
        record.pending && record.pending.previous === undefined &&
        record.anchorPath === undefined &&
        isGeneratedSidePath(artifactPath, record.pending.tempPath, ["tmp"]) &&
        inspectArtifact(fs, record.pending.tempPath).status === "absent";
      const staleUpdateProvenanceGone =
        staleUpdateProvenanceIsAbsent(fs, record, artifactPath);
      if (!activeAnchorIsAbsent && !firstInstallPendingIsAbsent &&
          !staleUpdateProvenanceGone) {
        try { fs.unlinkSync(tmp); } catch { /* unreferenced random temp */ }
        return { status: "deferred" };
      }
      // No public inode and no surviving owned side path remain. Retaining this
      // stale record would defer restoration forever; forgetting it is safe
      // because there is no existing file whose ownership could be misclassified.
      delete state.artifacts[artifactPath];
      record = undefined;
    }
...
function integrateLinuxDesktop(deps) {
  ...
  for (const size of ICON_SIZES) {
    ...
    try {
      ...
      if (inspectArtifact(fs, dest).status === "file") available += 1;
